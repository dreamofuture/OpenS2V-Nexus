...
class MaskDictionaryModel:
    ...
    def add_new_frame_annotation(
        self, mask_list, box_list, label_list, background_value=0
    ):
        ...
        for idx, (mask, box, label) in enumerate(zip(mask_list, box_list, label_list)):
            ...
            if mask.shape[0] != mask_img.shape[0] or mask.shape[1] != mask_img.shape[1]:
                raise ValueError(
                    "The mask shape should be the same as the mask_img shape."
                )
            # mask = mask
            mask_img[mask] = final_index
            # print("label", label)
            name = label
    ...
    def update_masks(
        self, tracking_annotation_dict, iou_threshold=0.8, objects_count=0
    ):
        updated_masks = {}
        ...
        for seg_obj_id, seg_mask in self.labels.items():  # tracking_masks
            ...
            for (
                object_id,
                object_info,
            ) in tracking_annotation_dict.labels.items():  # grounded_sam masks
                iou = self.calculate_iou(
                    seg_mask.mask, object_info.mask
                )  # tensor, numpy
                # print("iou", iou)
    ...
    def save_empty_mask_and_json(
        self, mask_data_dir, json_data_dir, image_name_list=None
    ):
        ...
        if image_name_list:
            for image_base_name in image_name_list:
                ...
                np.save(
                    os.path.join(mask_data_dir, mask_name),
                    mask_img.numpy().astype(np.uint16),
                )

                json_data_path = os.path.join(
                    json_data_dir, mask_name.replace(".npy", ".json")
                )
                print("save_empty_mask_and_json", json_data_path)
                self.to_json(json_data_path)
        else:
            np.save(
                os.path.join(mask_data_dir, self.mask_name),
                mask_img.numpy().astype(np.uint16),
            )
            json_data_path = os.path.join(
                json_data_dir, self.mask_name.replace(".npy", ".json")
            )
            print("save_empty_mask_and_json", json_data_path)
            self.to_json(json_data_path)
