...
def object_detection_and_segmentation(
    ...
    assert text_input is None, (
        "Text input should be None when calling object detection pipeline."
    )
    ...
    cv2.imwrite(
        os.path.join(output_dir, "grounded_sam2_florence2_det_annotated_image.jpg"),
        annotated_frame,
    )
    ...
    cv2.imwrite(
        os.path.join(output_dir, "grounded_sam2_florence2_det_image_with_mask.jpg"),
        annotated_frame,
    )
...
def dense_region_caption_and_segmentation(
    ...
    assert text_input is None, (
        "Text input should be None when calling dense region caption pipeline."
    )
    # run florence-2 object detection in demo
    image = Image.open(image_path).convert("RGB")
    results = run_florence2(
    ...
    cv2.imwrite(
        os.path.join(
            output_dir, "grounded_sam2_florence2_dense_region_cap_annotated_image.jpg"
        ),
        annotated_frame,
    )
    ...
    cv2.imwrite(
        os.path.join(
            output_dir, "grounded_sam2_florence2_dense_region_cap_image_with_mask.jpg"
        ),
        annotated_frame,
    )
...
def region_proposal_and_segmentation(
    ...
    assert text_input is None, (
        "Text input should be None when calling region proposal pipeline."
    )
    # run florence-2 object detection in demo
    image = Image.open(image_path).convert("RGB")
    results = run_florence2(
    ...
    # specify labels
    labels = [f"region_{idx}" for idx, class_name in enumerate(class_names)]
    ...
    cv2.imwrite(
        os.path.join(output_dir, "grounded_sam2_florence2_region_proposal.jpg"),
        annotated_frame,
    )
    ...
    cv2.imwrite(
        os.path.join(
            output_dir, "grounded_sam2_florence2_region_proposal_with_mask.jpg"
        ),
        annotated_frame,
    )
...
def phrase_grounding_and_segmentation(
    ...
    assert text_input is not None, (
        "Text input should not be None when calling phrase grounding pipeline."
    )
    results = results[task_prompt]
    # parse florence-2 detection results
    input_boxes = np.array(results["bboxes"])
    ...
    cv2.imwrite(
        os.path.join(output_dir, "grounded_sam2_florence2_phrase_grounding.jpg"),
        annotated_frame,
    )
    ...
    cv2.imwrite(
        os.path.join(
            output_dir, "grounded_sam2_florence2_phrase_grounding_with_mask.jpg"
        ),
        annotated_frame,
    )
...
def referring_expression_segmentation(
    ...
    """
    assert text_input is not None, (
        "Text input should not be None when calling referring segmentation pipeline."
    )
    results = results[task_prompt]
    # parse florence-2 detection results
    ...
    img = cv2.imread(image_path)
    detections = sv.Detections(
        xyxy=input_boxes, mask=florence2_mask.astype(bool), class_id=class_ids
    )
    ...
    cv2.imwrite(
        os.path.join(output_dir, "florence2_referring_segmentation_box.jpg"),
        annotated_frame,
    )
    ...
    cv2.imwrite(
        os.path.join(output_dir, "florence2_referring_segmentation_box_with_mask.jpg"),
        annotated_frame,
    )
    ...
    img = cv2.imread(image_path)
    detections = sv.Detections(
        xyxy=input_boxes, mask=sam2_masks.astype(bool), class_id=class_ids
    )
    ...
    cv2.imwrite(
        os.path.join(output_dir, "grounded_sam2_florence2_referring_box.jpg"),
        annotated_frame,
    )
    ...
    cv2.imwrite(
        os.path.join(
            output_dir, "grounded_sam2_florence2_referring_box_with_sam2_mask.jpg"
        ),
        annotated_frame,
    )
...
def open_vocabulary_detection_and_segmentation(
    ...
    cv2.imwrite(
        os.path.join(
            output_dir, "grounded_sam2_florence2_open_vocabulary_detection.jpg"
        ),
        annotated_frame,
    )
